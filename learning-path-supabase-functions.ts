// Learning Path Supabase Functions
// Use these functions in your React component to save learning paths
 ...
 */
export async function updateLearningPath(learningPathId: string, data: LearningPathData) {
  try {
    // First delete existing data (cascade will handle it)
    await deleteLearningPath(learningPathId, data.mentorId);

    // Then insert as new (or you can implement a more sophisticated update)
    return await saveLearningPath(data);
  } catch (error: any) {
