...
export class DataService {
  ...
  static async getStudentAssessments(studentId: string): Promise<ExtendedAssessment[]> {
    ...
    try {
      // First try the new student_assessments table
      let { data: studentAssessments, error: studentAssessmentsError } = await supabase
        ...
        `)
        .eq('student_id', studentId)
        .in('assessment.status', ['active', 'published', 'draft'])
        .order('assigned_at', { ascending: false });
      
      // If status filter fails, try without status filter
      if (studentAssessmentsError && studentAssessmentsError.message.includes('status')) {
        console.log('⚠️ Status column not found in student_assessments, trying without status filter');
        const { data: allStudentAssessments, error: allError } = await supabase
          .from('student_assessments')
          .select(`
            *,
            assessment:assessments(
        ...
        studentAssessments = allStudentAssessments;
        studentAssessmentsError = allError;
      }
      
      if (studentAssessmentsError) {
        console.warn('⚠️ student_assessments table not found, falling back to enrollment-based method');
        // The fallback logic is now handled in the empty studentAssessments case below
      }
      
      if (!studentAssessments || studentAssessments.length === 0) {
        console.log('⚠️ No assessments assigned to this student, falling back to enrollment-based method');
        
        // Fallback to the old method using enrollments with status check
        ...
        if (!enrollments || enrollments.length === 0) {
          console.log('⚠️ Student not enrolled in any courses or no active enrollments');
          return [];
        }
        ...
        // Get assessments for enrolled courses that are available to students
        ...
        console.log('✅ Student assessments fetched successfully (fallback method):', assessments?.length || 0);
        console.log('📊 Raw assessment data:', assessments);
      ...
      // Extract assessments from the joined data
      const assessments = studentAssessments
        .map(sa => sa.assessment)
        .filter(assessment => assessment !== null);
      
      console.log('✅ Student assessments fetched successfully (new method):', assessments.length);
      console.log('📚 Student assessment details:', assessments.map(a => ({
        id: a.id,
        title: a.title,
  ...
  static async getAssessmentDetails(assessmentId: string, studentId?: string): Promise<any> {
    try {
      ...
      // Get topics from assessment tags or course tags
      const topics = assessment.tags || assessment.course?.tags || ['General Topics'];
    ...
    try {
      ...
      if (questions && questions.length > 0) {
        ...
        if (questionsError) {
          ...
          // Don't throw error here, assessment was created successfully
        } else {
          console.log('✅ Questions created:', createdQuestions?.length || 0);
        }
      }

      // 3. Create student assignments for enrolled students
      const { data: enrollments, error: enrollmentError } = await supabase
        .from('enrollments')
        .select('student_id')
      ...
      if (!enrollmentError && enrollments && enrollments.length > 0) {
        const studentAssignments = enrollments.map(enrollment => ({
          ...
          assessment_id: assessment.id,
          assigned_by: assessmentData.instructor_id
        }));
        ...
        if (assignmentError) {
          console.warn('⚠️ Error creating student assignments:', assignmentError);
        } else {
          console.log('✅ Student assignments created:', studentAssignments.length);
        }
      }
