...
export default function AIAutomationModal({ isOpen, onClose }: AIAutomationModalProps) {
  ...
  // Debug logging
  console.log('AIAutomationModal - isOpen:', isOpen);
    ...
    <AnimatePresence>
      {isOpen && (
        <div style={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', background: 'rgba(0,0,0,0.8)', zIndex: 9999, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
          <div style={{ background: 'white', padding: '20px', borderRadius: '10px', maxWidth: '500px', width: '90%' }}>
            <h2>AI Automation Modal is Working!</h2>
            <p>This is a test to see if the modal is rendering.</p>
            <button onClick={onClose} style={{ background: 'blue', color: 'white', padding: '10px 20px', border: 'none', borderRadius: '5px', cursor: 'pointer' }}>
              Close Modal
            </button>
          </div>
        </div>
