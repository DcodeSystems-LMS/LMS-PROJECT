-- Complete Assessment Database Setup
...
CREATE TABLE IF NOT EXISTS assessments (
  ...
  bloom_taxonomy_level VARCHAR(20) DEFAULT 'remember',
...
CREATE TABLE IF NOT EXISTS student_accessibility_settings (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  student_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  settings JSONB DEFAULT '{}',
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
...
ALTER TABLE student_accessibility_settings ENABLE ROW LEVEL SECURITY;
...
CREATE POLICY "Mentors can manage questions for their assessments" ON questions
  FOR ALL USING (
    EXISTS (
      SELECT 1 FROM assessments 
      WHERE id = questions.assessment_id AND mentor_id = auth.uid()
    ) OR
    EXISTS (
      SELECT 1 FROM user_roles 
      WHERE user_id = auth.uid() AND role = 'admin'
...
CREATE POLICY "Mentors can view question analytics for their assessments" ON question_analytics
  FOR SELECT USING (
    EXISTS (
      SELECT 1 FROM assessments 
...
CREATE POLICY "Students can manage their accessibility settings" ON student_accessibility_settings
  FOR ALL USING (student_id = auth.uid());
...
-- Function to calculate question difficulty
...
CREATE INDEX IF NOT EXISTS idx_assessments_mentor_status ON assessments(mentor_id, status);
...
CREATE INDEX IF NOT EXISTS idx_time_tracking_student ON student_time_tracking(student_id);
CREATE INDEX IF NOT EXISTS idx_plagiarism_reports_student ON plagiarism_reports(student_id);
CREATE INDEX IF NOT EXISTS idx_leaderboards_assessment ON leaderboards(assessment_id);
...
INSERT INTO assessments (
  ...
  (SELECT id FROM auth.users WHERE role = 'mentor' LIMIT 1),
  30,
  3,
  70,
  ARRAY['sample', 'test'],
