-- Create Proper Assessment Database Schema (Safe Version)
-- This script safely creates the correct relational structure for assessments
...
DROP POLICY IF EXISTS "Mentors can manage questions for their assessments" ON questions;
DROP POLICY IF EXISTS "Students can view questions for assigned assessments" ON questions;
DROP POLICY IF EXISTS "Students can view their assigned assessments" ON student_assessments;
DROP POLICY IF EXISTS "Mentors can assign assessments to students" ON student_assessments;
DROP POLICY IF EXISTS "Mentors can update their assigned assessments" ON student_assessments;
DROP POLICY IF EXISTS "Students can manage their own attempts" ON assessment_attempts;
DROP POLICY IF EXISTS "Mentors can view attempts for their assessments" ON assessment_attempts;
DROP POLICY IF EXISTS "Students can manage their own responses" ON assessment_responses;
DROP POLICY IF EXISTS "Mentors can view responses for their assessments" ON assessment_responses;
...
-- 8. Create RLS policies for student_assessments
...
-- 9. Create RLS policies for assessment_attempts
...
-- 10. Create RLS policies for assessment_responses
...
-- 12. Create functions for updating timestamps
...
BEGIN
    ...
    FOR assessment_record IN
        ...
        AND questions::text != '""'
    LOOP
        -- Parse the JSON questions data
        question_data := assessment_record.questions::JSONB;
        
        -- Check if it's an array
        IF jsonb_typeof(question_data) = 'array' THEN
        ...
        ELSE
            ...
            RAISE NOTICE 'Skipping assessment % - questions data is not an array (type: %)', 
                assessment_record.id, jsonb_typeof(question_data);
        END IF;
    END LOOP;

    RAISE NOTICE 'Migrated % questions from JSON to questions table', question_count;                                                                           
END $$;

-- 15. Create student assignments for existing assessments
...
DO $$
BEGIN
    RAISE NOTICE 'Assessment schema setup completed successfully!';
    RAISE NOTICE 'Tables created: questions, student_assessments, assessment_attempts, assessment_responses';
